...
def ReadPacket():
    ...
    if (i > len(packet)):
        print("--- Beyond size of packet ---")    

import pyshark

# Define the BPF filter to capture packets from ports 25565 or 54862
# Capture packets from the specified pcap file with the filter
cap = pyshark.FileCapture('example/connection.pcap')
#cap.set_display_filter('tcp port 25565 or tcp port 54862')
...
for packetIndex,packet in enumerate(cap):
        ...
        if 'TCP' in packet:
            try:
                if (len(packet.tcp.payload) > 0): 
                    print(f"[{packetIndex}]", end='\t')
                    if (packet.tcp.port == clientPort):
                        PrintBold("CLIENT")
                        origin = Origin.Client
                    elif (packet.tcp.port == serverPort):
                        PrintBold("SERVER")
                        origin = Origin.Server
                    else:
                        print(f"{packet.tcp.port}: ")
                        origin = Origin.Client
